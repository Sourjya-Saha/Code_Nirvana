...
def get_all_products_global(connection):
    ...
    # Execute the query without any user_id filter
...
@app.route('/getAllProductsGlobal', methods=['GET'])
def get_all_products_route():
    try:
        connection = get_db()
        response = get_all_products_global(connection)
...
def edit_product():
    try:
        ...
        updated_product = {
            ...
            'picture_of_the_prod': request_payload['picture_of_the_prod'],
...
def fetch_drivers_assigned():
    try:
        ...
        # Query to fetch driver_id from transactions table
        ...
        # Query to fetch generated_unique_id and user_name from users_app table
...
def insert_product_whole():
    ...
    # Pass the user_id along with the product details to the insert_new_product function
    product_id = products_whole_dao.insert_new_product_whole(connection, request_payload, user_id)
...
def edit_product_whole():
    ...
    try:
        ...
        # Call the edit_product function from products_dao and pass the connection, product_id, and updated_product
        rows_affected = products_whole_dao.edit_product_whole(connection, product_id, updated_product)
...
def get_deviation_data():
    try:
        print("Received request to /get_deviation_data")  # Debugging the route hit

        # Get the cart_id from the POST request
        data = request.get_json()
        print(f"Request data: {data}")  # Debugging request payload

        cart_id = data.get('cart_id')
        print(f"Extracted cart_id: {cart_id}")  # Debugging cart_id extraction

        if not cart_id:
            print("cart_id is missing in the request")  # Debugging missing cart_id
            return jsonify({"error": "cart_id is required"}), 400

        # Connect to the database
        db = get_db()
        cursor = db.cursor(dictionary=True)
        print("Database connection established")  # Debugging DB connection

        # Fetch the deviation data
        query = "SELECT deviation_data FROM route_status WHERE cart_id = %s"
        print(f"Executing query: {query} with cart_id: {cart_id}")  # Debugging query execution
        cursor.execute(query, (cart_id,))
        result = cursor.fetchone()
        print(f"Query result: {result}")  # Debugging query result

        if not result or 'deviation_data' not in result:
            print(f"No deviation data found for cart_id: {cart_id}")  # Debugging missing data
            return jsonify({"error": "No deviation data found for the given cart_id"}), 404

        deviation_data = json.loads(result['deviation_data'])
        print(f"Parsed deviation data: {deviation_data}")  # Debugging parsed data

        # Organize the data
        organized_data = {}
        for index, item in enumerate(deviation_data):
            print(f"Processing deviation data at index: {index}")  # Debugging data loop
            organized_data[index] = {
                ...
                "end": {
                    "timestamp": deviation_data[-(index + 1)]['end']['timestamp'],
                    "location": deviation_data[-(index + 1)]['end']['location'],
                    "distance": deviation_data[-(index + 1)]['end']['distance']
                }
            }
            print(f"Organized item for index {index}: {organized_data[index]}")  # Debugging organized item

        print("Final organized data:", organized_data)  # Debugging final data
        return jsonify(organized_data)

    except Exception as e:
        print(f"Exception occurred: {e}")  # Debugging exception
        return jsonify({"error": str(e)}), 500

    finally:
        if 'cursor' in locals():
            cursor.close()
            print("Database cursor closed")  # Debugging cursor cleanup

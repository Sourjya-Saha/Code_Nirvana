...
def get_order_details(connection, order_id):
    ...
    query = """
        SELECT retailer_to_manu_details.order_id, products_manu.id, retailer_to_manu_details.quantity, retailer_to_manu_details.total_price, 
               products_manu.name AS product_name, products_manu.price_per_unit
        FROM retailer_to_manu_details
        LEFT JOIN products_manu ON retailer_to_manu_details.product_id = products_manu.id
        WHERE retailer_to_manu_details.order_id = %s
    """
...
def get_all_orders_retailer(connection, user_id):
    """
    ...
    for order in orders:
        order_dict = {'order_id': order[0], 'customer_name': order[1], 'datetime': order[2], 'phone_num': order[3],
                      'status': order[4], 'total': order[5], 'manu_id': order[6], 'order_details': get_order_details(connection, order[0])}
...
def get_all_orders_manu(connection, manu_id):
    ...
    for order in orders:
        order_dict = {'order_id': order[0], 'customer_name': order[1], 'datetime': order[2], 'phone_num': order[3],
                      'status': order[4], 'total': order[5], 'user_id': order[6], 'order_details': get_order_details(connection, order[0])}
...
def delete_transaction(connection, order_id):
    ...
    try:
        ...
        delete_order_details_query = "DELETE FROM retailer_to_manu_details WHERE order_id = %s"
        cursor.execute(delete_order_details_query, (order_id,))

        # Step 2: Delete the order from orders_table
        delete_order_query = "DELETE FROM retailer_to_manu WHERE order_id = %s"
...
def razorpay_order_ret_to_manu(connection, cart_id):
    ...
    try:
        ...
        # Step 2: Fetch order details from retailer_to_manu
        fetch_order_query = """
...
def update_order_status_transactions_ret_to_manu(connection, order_id):
    """
    ...
    try:
        ...
        update_query = "UPDATE retailer_to_manu SET status = 'paid' WHERE order_id = %s"

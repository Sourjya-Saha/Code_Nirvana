...
def get_all_products_whole(connection, user_id):
    ...
    response = []
    for (id, name, price_per_unit, uom_name, quantity_of_uom, category, exp_date, shelf_num, picture_of_the_prod, description, reference_cart) in cursor:
        response.append({
            'product_id': id,
            'name': name,
...
def edit_product_whole(connection, product_id, updated_product):
    cursor = connection.cursor()
    query = ("UPDATE products_whole SET "
             "name = %s, "
             "price_per_unit = %s, "
             "quantity_of_uom = %s, "
             "category = %s, "

...
def get_all_products(connection, user_id):
    ...
    for (id, name, price_per_unit, uom_name, quantity_of_uom, category, exp_date, shelf_num, picture_of_the_prod,
         description, reference_cart) in cursor:
        # Parse reference_cart JSON if it exists
        try:
...
def insert_new_product(connection, product, user_id, cart_id=None):
    ...
    # Initialize reference cart
    if cart_id:
        reference_cart = {
            "0": {
                "cart_id": cart_id,
                "quantity": product.get('cart_quantity', product.get('quantity_of_uom', 0)),
                "exp_date": product['exp_date'].isoformat() if product.get('exp_date') else None
            }
        }
    else:
        # If reference_cart is already provided in product_data, use it
        reference_cart = product.get('reference_cart', {})
        if isinstance(reference_cart, str):
            try:
                reference_cart = json.loads(reference_cart)
            except json.JSONDecodeError:
                reference_cart = {}
    ...
    data = (
        product['name'],
        product.get('uom_id'),  # Using get() to handle potential None values
        product.get('price_per_unit'),
        product.get('quantity_of_uom', 0),  # Default to 0 if not provided

...
function Products() {
  ...
  const fetchData = async () => {
    try {
      ...
      setErrorMessage('Something went wrong fetching products.');
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      setErrorMessage('Error occurred while updating product.');
    }
  };
  ...
  const formatDateForInput = (dateStr) => {
    ...
    // ISO string like "2024-11-06T00:00:00"
    if (dateStr.includes('T')) return dateStr.split('T')[0];
    // GMT string like "Wed, 06 Nov 2024 00:00:00 GMT"
  ...
  const openEditModal = (prod) => {
    ...
    if (rawDate) {
      if (/^\d{4}-\d{2}-\d{2}$/.test(rawDate)) {
        ...
        if (!isNaN(parsed)) normalizedDate = parsed.toISOString().split("T")[0];
      }
    }
    setSelectedProduct({ ...prod, exp_date: normalizedDate });
    setIsEditing(true);
  };
    ...
    <>
        ...
        <div className="grid-card wh-table-card">
          ...
          <div className="wh-toolbar">
            ...
            <div className="toolbar-actions">
              <button className="scan-button toolbar-btn" onClick={() => navigate('/addproducts')}>
            ...
            <table className="transactions-table" id="product-table">
              ...
              <tbody>
                {currentProducts.length === 0 ? (
                  <tr>
                    <td colSpan="10" className="empty-table-msg">No products found.</td>
                  </tr>
                  ...
                  <tr key={product.product_id} className="transaction-row">
                    ...
                    <td>
                      {product.picture_of_the_prod ? (
                        ...
                        <span style={{ color: '#9ca3af', fontSize: '0.82rem' }}>No Image</span>
                      )}
                    </td>
              ...
              <div className="modal-card-body">
                <div className="modal-form-grid">
                  ...
                  <div className="form-group">
                    ...
                    <input className="form-input" type="date" name="exp_date"
                  ...
                  {selectedProduct.picture_of_the_prod && (
                    <div style={{ marginBottom: '10px', textAlign: 'center' }}>
                      <img
                        ...
                        alt="Preview"
                        style={{ width: '140px', borderRadius: '10px', objectFit: 'cover' }}
                      />
                    </div>
                  )}
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setSelectedProduct((prev) => ({ ...prev, picture_of_the_prod: file }));
                    }}
                  />
                </div>
              </div>
                ...
                {selectedProduct.picture_of_the_prod && (
                  <div style={{ textAlign: 'center', marginBottom: '18px' }}>
                    <img
                      ...
                      alt="Product"
                      style={{ width: '180px', borderRadius: '12px', objectFit: 'cover', boxShadow: '0 10px 25px rgba(0,0,0,0.15)' }}
                    />
                  </div>
                )}
      ...
      <style>{`
        ...
        .toolbar-btn {
          ...
          border-radius: 8px; border: none; font-weight: 700; font-size: 0.95rem;
          font-family: 'Poppins', sans-serif; cursor: pointer; transition: all 0.3s ease;
          text-transform: uppercase; letter-spacing: 0.05em;
        }
        .scan-button { background-color: #f59e0b; color: white; }
        .scan-button:hover { background-color: #d97706; transform: translateY(-2px); }
        .validate-after-payment { background-color: #8b5cf6; color: white; }
        .validate-after-payment:hover { background-color: #7c3aed; transform: translateY(-2px); }
        ...
        .id-chip { background: #ede9fe; color: #7c3aed; padding: 4px 10px; border-radius: 6px; font-size: 0.85rem; font-weight: 700; }
        ...
        .table-product-image { max-width: 90px; max-height: 80px; object-fit: contain; border-radius: 8px; }
        .status-badge {
        ...
        .form-group label { font-weight: 600; color: #4b5563; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.05em; }
        .form-input {
          padding: 12px 16px; border-radius: 8px; border: 1px solid #d1d5db; font-size: 0.95rem;
        ...
        .modal-action-btn {
          display: flex; align-items: center; gap: 8px; padding: 13px 28px; border-radius: 8px; border: none;
          font-weight: 700; font-size: 0.95rem; font-family: 'Poppins', sans-serif; cursor: pointer;
          transition: all 0.3s ease; text-transform: uppercase; letter-spacing: 0.05em;
        }
        .payment-button { background-color: #10b981; color: white; }
        ...
        .detail-item {
          ...
          padding: 12px 16px; display: flex; flex-direction: column; gap: 6px; animation: fadeIn 0.4s ease;
        }
        @keyframes fadeIn {
          from { opacity: 0; transform: translateY(6px); }
          to { opacity: 1; transform: translateY(0); }
        }
        .detail-label { font-size: 0.8rem; color: #6b7280; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; }

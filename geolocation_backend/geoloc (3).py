...
class DatabaseManager:
    ...
    def get_shipment_participants(cart_id: str) -> Dict[str, str]:
        ...
        try:
            with DatabaseManager.get_connection() as conn:
                ...
                query1 = """
                    SELECT manu_add, whole_add, ret_add, driver_id, distributor_id  
                    FROM transactions 
                    WHERE cart_id = %s
                """
    ...
    def verify_shipment_access(cart_id: str, user_id: str, role: str) -> bool:
        ...
        try:
            with DatabaseManager.get_connection() as conn:
                ...
                if role == 'driver' or role == 'distributor':
                    driver_query = """
                            SELECT 1
                            FROM transactions
                            WHERE (cart_id = %s AND driver_id = %s) OR (cart_id = %s AND distributor_id = %s)
                            LIMIT 1
                        """
                    cursor.execute(driver_query, (cart_id, user_id, cart_id, user_id))
                    if cursor.fetchone():
                        return True
    ...
    @staticmethod
    def get_shipment_details(cart_id: str, user_id: str, role: str) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            role: The user's role ('distributor' or other roles)

        Returns:
            Dict containing shipment details or None if not found/error occurs
        """
        try:
            print(f"Attempting to fetch shipment details for cart_id: {cart_id}, user_id: {user_id}, role: {role}")
            ...
            with DatabaseManager.get_connection() as conn:
                ...
                user_query = """
                    ...
                    WHERE generated_unique_id = %s
                """
                print(f"Executing user query for user_id: {user_id}")
                cursor.execute(user_query, (user_id,))
                user_result = cursor.fetchone()
                ...
                user_address = user_result['metamask_add']
                print(f"Fetched user address: {user_address}")

                # Base query for shipment details
                base_query = """
                    SELECT t.*,
                           r.current_location,
                           r.status,
                ...
                if role == 'distributor':
                    access_condition = "AND t.distributor_id = %s"
                    params = (cart_id, user_id)
                    print(f"Role is distributor, access_condition: {access_condition}")
                else:
                    access_condition = """
                        AND (t.manu_add = %s 
                             OR t.whole_add = %s 
                             ...
                             OR t.driver_id = %s)
                    """
                    params = (cart_id, user_address, user_address, user_address, user_id)
                    print(f"Role is not distributor, access_condition: {access_condition}")

                # Combine the queries
                final_query = base_query + access_condition
                print(f"Executing final query: {final_query} with params: {params}")
                cursor.execute(final_query, params)
                result = cursor.fetchone()

                if result:
                    print(f"Shipment details found: {result}")
                else:
                    print("No shipment details found.")
...
class StateManager:
    def __init__(self):
        ...
        self.chat_history: Dict[str, List[Dict[str, Any]]] = {}  # Maps cart_id to list of messages
    ...
    # Add new method for managing chat history
    def add_message(self, cart_id: str, message: Dict[str, Any]) -> bool:
        """
        Add a message to the chat history for a specific cart.

        Args:
            cart_id: The cart identifier
            message: Dictionary containing message details (sender_id, sender_name, content, timestamp)
        ...
        if cart_id not in self.chat_history:
            self.chat_history[cart_id] = []

        self.chat_history[cart_id].append(message)
        return True

    def get_chat_history(self, cart_id: str) -> List[Dict[str, Any]]:
        """Get chat history for a specific cart."""
        return self.chat_history.get(cart_id, [])

    def clear_chat_history(self, cart_id: str) -> None:
        """Clear chat history for a specific cart."""
        if cart_id in self.chat_history:
            del self.chat_history[cart_id]
...
def handle_driver_location(data):
    ...
    try:
        ...
        try:
            ...
            participant_values = [int(float(value)) for value in participants.values() if value is not None]
...
# Add new socket event handlers for chat functionality
...
def handle_message(data):
    ...
    if not all([cart_id, content, user_id]):
    ...
    try:
        ...
        participants = DatabaseManager.get_shipment_participants(cart_id)
        print(f"Participants for message: {participants}")

        # Convert user_id to int for comparison
        try:
            user_id_int = int(user_id)
            participant_values = [int(float(value)) for value in participants.values() if value is not None]
            print(f"Checking message access - user_id: {user_id_int}, valid participants: {participant_values}")

            if not participants or user_id_int not in participant_values:
                print(f"Message access denied for user {user_id_int}")
                emit('message-error', {
            ...
            print(f"Error converting user IDs in message handler: {e}")
            emit('message-error', {
        ...
        try:
            with DatabaseManager.get_connection() as conn:
                ...
                if not result or result['status'] != 'ACTIVE':
                    print(f"Route not active for cart_id: {cart_id}")
                    emit('message-error', {
            ...
            print(f"Database error checking route status: {e}")
            raise
        ...
        try:
            with DatabaseManager.get_connection() as conn:
                ...
                if role in ['manufacturer', 'retailer', 'wholesaler']:
                    query = "SELECT user_id FROM users WHERE generated_unique_id = %s"
                    cursor.execute(query, (user_id,))
                    user_result = cursor.fetchone()
                    username = user_result['user_id'] if user_result else 'Unknown User'
                elif role in ['driver', 'distributor']:
                    query = "SELECT user_name FROM users_app WHERE generated_unique_id = %s"
                    cursor.execute(query, (user_id,))
                    user_result = cursor.fetchone()
                    username = user_result['user_name'] if user_result else 'Unknown User'
        ...
        message = {
            'sender_id': str(user_id),  # Ensure consistent string type for sender_id
            'sender_name': username,
            'content': content.strip(),  # Remove any leading/trailing whitespace
            'timestamp': datetime.now(timezone.utc).isoformat(),
            'cart_id': cart_id,
            'role': role  # Include sender's role
        }
        print(f"Created message object: {message}")

        # Add message to history
        if state.add_message(cart_id, message):
            # Broadcast message to room
            print(f"Broadcasting message to room {cart_id}")
            emit('new-message', message, to=cart_id)
            ...
            print("Message exceeded maximum length")
            emit('message-error', {
        ...
        emit('message-error', {
            ...
            'cart_id': cart_id if 'cart_id' in locals() else None
        }, to=request.sid)
    except Exception as e:
...
@socketio.on('request-chat-history')
def handle_chat_history_request(data):
    """Send chat history to newly connected clients."""
    cart_id = data.get('cart_id')
    user_info = state.connected_users.get(request.sid, {})
    user_id = user_info.get('user_id')
    print(f"cart_id: {cart_id}, user_info: {user_info}, user_id: {user_id}")

    # Verify user has access
    participants = DatabaseManager.get_shipment_participants(cart_id)
    print(f"Participants: {participants}")

    # Convert user_id to int for comparison since it might come as string
    try:
        user_id_int = int(user_id)
        participant_values = [int(float(value)) for value in participants.values() if value is not None]
        print(f"Checking access - user_id: {user_id_int}, valid participants: {participant_values}")

        if not participants or user_id_int not in participant_values:
            print(f"Access denied for user {user_id_int}")
            return
    ...
    # Send chat history
    chat_history = state.get_chat_history(cart_id)
    print(f"Sending chat history: {chat_history}")
...
def verify_shipment_access():
    ...
    if has_access:
        ...
        shipment = DatabaseManager.get_shipment_details(cart_id, user_id, role)
        return jsonify({
            "success": True,
            "authorized": True,
            "shipment": shipment
        ...
        if distance_to_final <= 50:
            try:
                with conn.cursor() as cursor:
                    ...
                    # Clear chat history when route is completed
                    state.clear_chat_history(cart_id)

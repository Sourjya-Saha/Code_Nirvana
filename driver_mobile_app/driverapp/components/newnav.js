...
const getManeuverIcon = (instruction: string = ''): string => {
  ...
  if (instructionLower.includes('take exit') || instructionLower.includes('ramp')) return 'exit-to-app';
  if (instructionLower.includes('roundabout')) return 'rotate-right';
  if (instructionLower.includes('continue') || instructionLower.includes('straight')) return 'arrow-upward';
  if (instructionLower.includes('arrive')) return 'place';
  
  // Default icon for unknown maneuvers
  return 'directions';
};

const BOTTOM_SHEET_MAX_HEIGHT = height * 0.8;
const BOTTOM_SHEET_MIN_HEIGHT = 100; // Reduced minimum height
...
const TrackingScreen = () => {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        ...
        // Clamp the values to our defined limits
        if (newValue < MAX_UPWARD_TRANSLATE_Y) {
          newValue = MAX_UPWARD_TRANSLATE_Y;
        }
        if (newValue > MAX_DOWNWARD_TRANSLATE_Y) {
          newValue = MAX_DOWNWARD_TRANSLATE_Y;
        }
      ...
      onPanResponderRelease: (_, gestureState) => {
        ...
        // If velocity is high enough, snap to the direction of velocity
        if (Math.abs(vy) > 0.5) {
          if (vy > 0) {
            // Moving down
            snapBottomSheet('down');
          } else {
            // Moving up
            snapBottomSheet('up');
          }
        ...
        // Otherwise snap based on drag distance
        if (dy > DRAG_THRESHOLD) {
          ...
          snapBottomSheet(dy > 0 ? 'down' : 'up');
        }
      },
    })
  ).current;

  const snapBottomSheet = (direction: 'up' | 'down') => {
  ...
  const getAddressFromCoords = async (coords) => {
    try {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?latlng=${coords}&key=${GOOGLE_MAPS_API_KEY}`
      );
  ...
  const handleRouteData = async (route: Route) => {
    ...
    const startAddress = await getAddressFromCoords(route.metadata.startLocation);
    const checkpointAddress = await getAddressFromCoords(route.metadata.checkpoint);
    const destinationAddress = await getAddressFromCoords(route.metadata.destination);
  ...
  <View style={styles.routeDetailsContainer}>
    <View style={styles.routeHeaderContainer}>
      <View style={styles.routeHeaderLeft}>
        <MaterialIcons name="local-shipping" size={24} color="#1E2A78" />
        <Text style={styles.routeHeaderText}>Active Delivery</Text>
      </View>
      <Text style={styles.cartId}>{`#${cartId.slice(-6)}`}</Text>
    </View>
    ...
    <View style={styles.locationsList}>
      {[
        ...
        <View style={styles.locationIcon}>
          <View style={[styles.iconCircle, getIconBackground(location.color)]}>
      ...
      {currentRoute?.metadata ? (
        ...
        <View style={styles.noRouteContainer}>
          <View style={styles.noRouteIconContainer}>
            <MaterialIcons name="local-shipping" size={48} color="#CBD5E0" />
            <MaterialIcons name="location-off" size={32} color="#CBD5E0" style={styles.secondaryIcon} />
          </View>
          <Text style={styles.noRouteText}>No active deliveries</Text>
          <Text style={styles.noRouteSubText}>Check your shipments for new assignments</Text>
        </View>
      )}
    </ScrollView>
  </View>
  ...
  return (
    ...
    <Animated.View 
        style={[
          ...
          {
            transform: [{
              translateY: bottomSheetAnimation
            }]
          }
        ]}
...
const styles = StyleSheet.create({
  ...
  centerMapButton: {
    position: 'absolute',
    right: 16,
    bottom: BOTTOM_SHEET_MIN_HEIGHT + 20, // Adjusted to account for collapsed bottom sheet
  ...
  bottomSheetContent: {
    padding: 20,
    paddingTop: 30, // Increased to accommodate handle
    height: '100%',
  },
  bottomSheetHandle: {
    width: 40,
